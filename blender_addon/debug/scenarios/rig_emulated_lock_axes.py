# File: scenarios/rig_emulated_lock_axes.py
# Code: Claude Code
# Review: Ryoichi Ando (ryoichi.ando@zozo.com)
# License: Apache v2.0
#
# Every Lock Translation / Lock Rotation mode driven through the emulated
...
# emulator is not integrating at all, when gravity is zero, or when the
...
# The emulator is the point: it is the only CUDA-free implementation of the
...
def drive(project, setup, gravity, spin):
    """Run one sheet through the emulated solver and return its motion.
    ...
    if not session.finished():
        raise RuntimeError("emulated run did not finish")
...
def run(ctx: r.ScenarioContext) -> dict:
    ...
    # The emulator's advance is a no-op unless the validation-grade elastic is
    # on, and with it off nothing moves, so every locked case would pass and
    # every control would fail.
    env["PPF_EMULATED_ELASTIC"] = "1"
    # The per-step sleep exists so other scenarios can observe BUSY/RUNNING
    # transitions. Nothing here reads a transition, and seven runs at the
    # default would take minutes.
    ...
    return r.report_named_checks(cases, label="emulated lock cases")
